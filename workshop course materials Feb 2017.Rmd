...
[This website](http://www.maths.lancs.ac.uk/~rowlings/Teaching/UseR2012/cheatsheet.html) has a very useful style guide for working with `Spatial` objects. 
...
You can have a look at the created object using the `class` and `str` functions
...
newXY.st <- STIDF(newXY, time = cassdat.lt.WGS@data$date, data = data.frame(id=cassdat.lt.WGS@data[,c("id")]))
...
**ggplot2**, one of the packages in the tidyverse, has a similar look...
```{r, eval = FALSE}
exampledata  %>% 
...
There is also this awesome [Cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf) for ggplot2
